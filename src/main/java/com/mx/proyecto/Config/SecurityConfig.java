...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http // LOGIN VALIDO SIN DEJAR ACCEDER A OTRAS URLS SIN LOGGEARTE PREVIAMENTE Y
				...
				.authorizeRequests()
				.and()
				.formLogin().loginPage("/LoginService").permitAll()
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		auth.jdbcAuthentication().dataSource(datasource)
				.usersByUsernameQuery("SELECT USERNAME, PASSWORD, CASE HABILITADO WHEN 'Y' THEN 1 ELSE 2 END AS HABILITADO\r\n"
		        		+ "		                FROM PROYECTOSPRING.ACCESO_USUARIOS  WHERE USERNAME=?")
		        .authoritiesByUsernameQuery("SELECT U.USERNAME, "
		        		+ "R.PERMISO FROM PROYECTOSPRING.ACCESO_USUARIOS U, PROYECTOSPRING.ROLES_USUARIOS R WHERE U.ROLE_USER = R.ID_PERMISO "
		        		+ "AND U.USERNAME=?");
